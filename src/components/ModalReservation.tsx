...
import {
  Chef,
  ReservationProps,
  ReservationPropsCreate,
} from "../lib/interfaces";
import { UserProps } from "../lib/interfaces";
import TextErrorAlert, {
  TextErrorAlertProps,
} from "./ui/alerts/TextErrorAlert";
...
export default function ModalReservation({
  ...
  const [textErrorAlertProps, setTextErrorAlert] =
    useState<TextErrorAlertProps>({ text: "show", show: false });
  const [isLoadingBtn, setIsLoadingBtn] = useState<boolean>(false);
  const [restricaoSelecionada, setRestricaoSelecionada] = useState("");

  const [reservation, setReservation] = useState<ReservationPropsCreate>({
    customer_name: "",
    customer_cpf: "",
    customer_email: "",
    description: "",
  ...
  const getDateTime = () => {
    const date =
      reservation.date.getDate() < 10
        ? "0" + reservation.date.getDate()
        : reservation.date.getDate();
    const month =
      reservation.date.getMonth() + 1 < 10
  ...
  const validateStep1 = () => {
    ...
    if (
      ...
      setTextErrorAlert({
        text: "Preencha todos os dados do cliente!",
        show: true,
      });
      return false;
    }
    ...
    if (!reservation.date || !reservation.mealType) {
      setTextErrorAlert({
        text: "Preencha os detalhes do agendamento!",
        show: true,
      });
      return false;
    }
    ...
    if (reservation.guests === 0) {
      setTextErrorAlert({
        text: "Defina um número de convidados válido",
        show: true,
      });
      return false;
    }
    ...
    setReservation((prev) => ({
      ...prev,
      description:
        userData.nome + " - " + reservation.mealType + " - " + getDateTime(),
    }));
  ...
  const handleReservation = async () => {
    ...
    const payload = {
      ...reservation,
      customer_cpf: unmask(reservation.customer_cpf),
      phone: unmask(reservation.phone),
    };
    ...
    if (!result || !result.ok) {
      setTextErrorAlert({ text: response.message, show: true });
      return;
    }
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-4">
          <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-3xl max-h-[90vh] overflow-y-auto relative">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setReservation((prev) => ({ ...prev, phone: value }));
                        }}
                        className="w-full border border-gray-300 px-3 py-2 rounded text-gray-700"
                      />
                    </div>
                    ...
                    <div className="w-full col-span-2">
                      {" "}
                      {/* Isso faz o input ocupar as duas colunas em telas médias */}
                      <label
                        htmlFor="location"
                    ...
                    <div className="w-full">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <select
                          ...
                          onChange={(e) =>
                            setRestricaoSelecionada(e.target.value)
                          }
                ...
                <div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center mt-1">
                          <div className="w-12 h-12 bg-gray-200 rounded-full mr-3">
                            <img
                              src={selectedChef.user.profile_photo}
                              alt="Foto chef selecionado"
                            />
                          </div>

                          <div>
                            <p className="font-medium">
                              {selectedChef.user.name}
                            </p>
                            <div className="flex items-center">
                              <div className="flex">
                                {[...Array(5)].map((_, i) => (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <p className="font-medium mt-1">{reservation.location}</p>
                    </div>
            ...
            <div className="flex justify-between mt-10">
              ...
              <SubmitBtn
                title={
                  currentStep === 2 ? "Solicitar Agendamento" : "Continuar"
                }
                isLoading={isLoadingBtn}
                callback={
                  currentStep === 2 ? () => handleReservation() : handleNext
                }
              />
            </div>
          </div>
